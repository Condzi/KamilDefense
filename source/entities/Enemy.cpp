...
namespace kd
{
	...
	void Enemy::shoot()
	{
		this->shootTime = kd::settings_t::GetInstance().GAMEPLAY.ENEMY_SHOOT_COOLDOWN;
		...
		missileRight->SetPosition( { this->position.x + ( 4 * kd::settings_t::GetInstance().GAMEPLAY.SCALE ), this->position.y } );

		missileLeft->velocity.x = -kd::settings_t::GetInstance().GAMEPLAY.ENEMY_MISSILE_SPEED_X;
		missileRight->velocity.x = kd::settings_t::GetInstance().GAMEPLAY.ENEMY_MISSILE_SPEED_X;
